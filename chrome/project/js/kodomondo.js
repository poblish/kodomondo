	...
	if (!( term in set)) {
		...
		termLookupPort.onMessage.addListener( function(resp) {
			if (/* Match response to request */ resp.origTerm == term) {
				$('body').highlight( ioStats, ioHistory, inDocUrl, new HighlightClass({terms: [ resp.classDetails.name ], foundClass: resp.classDetails.name, jarFQN: resp.jarFQN, artifact: resp.artifact, className:'highlightCore'}), inOptions);
			}
		});
